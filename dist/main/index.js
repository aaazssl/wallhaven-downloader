...
async function fetchImageDetails(id, apiKey) {
  try {
    const url2 = `https://wallhaven.cc/api/v1/w/${id}`;
    ...
    if (response.data && response.data.data) {
      return {
        ...
        ext: path$1.extname(response.data.data.path) || ".jpg"
      };
    }
...
async function downloadImage(detail, saveDir, onProgress) {
  const { id, url: url2, ext } = detail;
  const filePath = path$1.join(saveDir, `${id}${ext}`);
  ...
  try {
    ...
    if (fs$1.existsSync(filePath)) fs$1.unlinkSync(filePath);
    throw error;
  }
}
async function downloadManager(imageIds, downloadDir, concurrency, progressCallback) {
  const results = {
...
const historyFile = path$1.join(userDataPath, "history.json");
const configFile = path$1.join(userDataPath, "config.json");
...
function saveHistory(history) {
  try {
    fs$1.writeFileSync(historyFile, JSON.stringify(history, null, 2));
  } catch (error) {
...
function loadConfig() {
  ...
  try {
    if (fs$1.existsSync(configFile)) {
      const data = fs$1.readFileSync(configFile, "utf8");
...
function saveConfig(config) {
  try {
    fs$1.writeFileSync(configFile, JSON.stringify(config, null, 2));
  } catch (error) {
...
electron.ipcMain.handle("select-download-dir", async () => {
...
electron.ipcMain.handle("save-config", (event, config) => {
...
electron.ipcMain.handle("download-images", async (event, imageIds, downloadDir, concurrency) => {
