...
export default function Sidebar({
  ...
  const categoriesList = [
    ...
    { id: 'recent-hot', name: '近期热门', sort: 'hot', order: 'desc', showDateRange: false },
  ...
  // 注意：分区（浏览类型）按钮已迁移到顶部标签栏 components/TypeTabs.jsx，
  // 本组件不再负责分区切换（原 handleCategoryClick 已移除）

  const handleTopRangeChange = (range) => {
    setTopRange(range)
    // 时间范围变化时，只刷新 'toplist'（顶级列表）
    if (activeCategory === 'toplist') {
      onSelectCategory('toplist', 'desc', 'toplist', range)
    }
  }
